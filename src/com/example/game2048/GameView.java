...
public class GameView extends LinearLayout {//这里的GameView是继承自LinearLayout了
...
//		setBackgroundColor(0xffbbada0);//大背景色
		setBackgroundColor(0xffD3D2CC);
	...
	private void addCards(int cardWidth,int cardHeight){
		...
		LinearLayout line;//这里并没有指定line是水平排列还是垂直排列
		LinearLayout.LayoutParams lineLp;
		
		for (int y = 0; y < Config.LINES; y++) {
			line = new LinearLayout(getContext());
			lineLp = new LinearLayout.LayoutParams(-1, cardHeight);//-1是充满宽度的意思，第二个参数是高度值ֵ
			addView(line, lineLp);
	...
	private void addRandomNum(){
		...
		if (emptyPoints.size()>0) {
			...
			Point p = emptyPoints.remove((int)(Math.random()*emptyPoints.size()));//romove掉这个是因为赋值了之后就不为空了，同时也能返回位置
			cardsMap[p.x][p.y].setNum(Math.random()>0.1?2:4);

			MainActivity.getMainActivity().getAnimLayer().createScaleTo1(cardsMap[p.x][p.y]);//这里就是进行动画效果的地方了，这里是随机生成数字的动画效果
		}
	}
